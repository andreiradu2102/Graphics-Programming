...
vec3 SpotLight()
{
    ...
    if (spot_light > cos(cut_off)) {
        ...
        vec3 shadow_color = vec3(0.1, 0.0, 0.15);

        vec3 new_shadow = mix(shadow_color, light_att_factor * light_intensity * PhongLight(), s);

        return new_shadow + KA;
    }
