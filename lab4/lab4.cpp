...
void Lab4::OnKeyPress(int key, int mods)
{
    ...
    // number of instances: [ decreases, ] increases
    if (key == GLFW_KEY_LEFT_BRACKET) {
        if (no_of_instances > 1) no_of_instances--;
        std::cout << "instances = " << no_of_instances << "\n";
    }
    ...
    // number of generated points along the curve: - decreases, = increases
    if (key == GLFW_KEY_MINUS) {
        if (no_of_generated_points > 2) no_of_generated_points--;
        if (no_of_generated_points < 2) no_of_generated_points = 2;
        std::cout << "curve points = " << no_of_generated_points << "\n";
    }
    if (key == GLFW_KEY_EQUAL) {
        if (no_of_generated_points < 128) no_of_generated_points++;
        std::cout << "curve points = " << no_of_generated_points << "\n";
    }

    // T => translation surface (disable rotation); R => rotation surface (disable translation)
    if (key == GLFW_KEY_T) {
        max_translate = 8.0f;
        max_rotate = 0.0f;
        std::cout << "Surface = Translation\n";
    }
    if (key == GLFW_KEY_R) {
        max_translate = 0.0f;
        max_rotate = glm::radians(360.0f);
        std::cout << "Surface = Rotation\n";
    }
