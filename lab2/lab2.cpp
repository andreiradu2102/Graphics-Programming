...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    static std::unordered_map<const void*, glm::vec3> basePos;
    ...
    for (auto& l : lights)
    {
        ...
        if (basePos.find(key) == basePos.end())
            basePos[key] = l.position;

        const float amplitude = 2.0f;
        const float freqHz = 0.25f;
        const float omega = glm::two_pi<float>() * freqHz;

        const glm::vec3 base = basePos[key];
        l.position.y = base.y + amplitude * std::sin(omega * elapsedTime);
    ...
    // Lighting pass
    {
        ...
        for (auto& lightInfo : lights)
        {
            ...
            {
                int loc = shader->GetUniformLocation("light_position");
                glUniform3fv(loc, 1, glm::value_ptr(lightInfo.position));
            }
            {
                int loc = shader->GetUniformLocation("light_color");
                glUniform3fv(loc, 1, glm::value_ptr(lightInfo.color));
            }
            {
                int loc = shader->GetUniformLocation("light_radius");
                glUniform1f(loc, lightInfo.radius);
            }
