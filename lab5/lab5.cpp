...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    if (scene == 0)
    {
        ...
        if (shader->GetProgramID())
        {
            ...
            int loc;
            loc = glGetUniformLocation(shader->GetProgramID(), "generator_position");
            glUniform3f(loc, generator_position.x, generator_position.y, generator_position.z);
            loc = glGetUniformLocation(shader->GetProgramID(), "deltaTime");
            glUniform1f(loc, deltaTimeSeconds);
            loc = glGetUniformLocation(shader->GetProgramID(), "offset");
